...
def home_team(stats, home, play_by_play, txt_posession): 
    lst_home_sort = []
...
def away_team(stats, home, play_by_play, txt_posession): 
    lst_away_sort = []
    for i in range(len(stats)):
        check = stats[i]["Players\t"]
...
def analyse_nba_game(play_by_play): 
    with open (play_by_play,'r', encoding='utf-8') as csv_text:
        ...
        each_play = [each for each in text_lines] 
...
analyse_nba_game("/home/docode/project/nba_game_warriors_thunder_20181016.txt")